...
import { Home, PlusCircle, Settings, X, Menu, LogOut, User, Bot } from 'lucide-react';
...
export function Sidebar() {
  ...
  const handleAiAssistantClick = (e: React.MouseEvent) => {
    e.preventDefault();
    // Dispatch custom event to notify parent about AI assistant click
                    ...
                    >
                      <motion.div
                        className={`group flex items-center rounded-lg transition-all duration-200 ${
                          false // isActive check is not applicable for AI Assistant since it doesn't navigate
